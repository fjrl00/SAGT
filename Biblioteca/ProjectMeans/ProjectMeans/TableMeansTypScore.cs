...
{
    public class TableMeansTypScore : InterfaceTableMeans
    {
        /*
         * Descripción:
         ...
         *      filas). El número de columnas es igual al número de facetas más 5.
         *      
        ...
        public TableMeansTypScore(ListFacets lF, string design, MultiFacetsObs mfo, bool zero)
        {
            ...
            int cols = lF.Count() + 5; // nº de columnas de la matriz de observaciones
        ...
        public TableMeansTypScore(ListFacets lf, string design, double? grandMean, double? variance, double? std_dev, 
            ...
            List<double?> listDifMeans, List<double?> listPointT)
        {
            if (lf.Count() < 1)
            {
                throw new ObsTableException("Error: no hay facetas");
            ...
            if (!(listMeans.Count == listStd_dev.Count && listMeans.Count == listVariances.Count
                && listDifMeans.Count == listDifMeans.Count && listMeans.Count == listPointT.Count))
            {
            ...
            for (int j = 0; j < n; j++)
            {
                meansMatrix[j][cols - 5] = listMeans[j];
                meansMatrix[j][cols - 4] = listVariances[j];
                meansMatrix[j][cols - 3] = listStd_dev[j];
                meansMatrix[j][cols - 2] = listDifMeans[j];
                meansMatrix[j][cols - 1] = listPointT[j];
            }
